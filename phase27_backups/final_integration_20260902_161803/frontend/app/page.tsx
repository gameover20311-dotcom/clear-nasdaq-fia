...
export default function Home() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await fetch(DASHBOARD_ENDPOINT, { cache: "no-store" });
    ...
    <section className="grid2">
      <Panel title="Liquidity Map" eyebrow="SWEEPS / RECLAIMS / KEY LEVELS">
        <div className="kvList">
          {entries(liq).length ? entries(liq).slice(0, 14).map(([k, raw]: any) => {
            const v: AnyObj = raw || {};
            return <div className="kv" key={k}><span>{v.name || titleize(k)} <small>{v.status || "—"}</small></span><strong>{v.price !== undefined && v.price !== null ? fmt(v.price, 2) : "—"}{v.distance_pct !== undefined && v.distance_pct !== null ? ` · ${fmt(v.distance_pct, 2)}%` : ""}</strong></div>;
          }) : <div className="empty">Liquidity data unavailable.</div>}
        </div>
      </Panel>
