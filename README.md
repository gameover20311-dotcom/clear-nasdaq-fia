...
The latest build adds the full cognitive evidence architecture under `backend/fia/cognitive/` and the strict replay/validation package under `backend/fia_backtest_phase30/`.

Primary live endpoints:

- `/api/cognitive/forecast?deep=false&horizon=8h`
- `/api/cognitive/forecast?deep=true&horizon=8h`
- `/api/cognitive/status`

The dashboard now exposes the cognitive direction, calibrated probability, reliability gate, regime, historical analogies, top specialist contributions and independent critic objections.

See `MASTER_COGNITIVE_AI_SPEC.md` for the locked architecture and acceptance rules. The build is research-only; no broker execution is included.
