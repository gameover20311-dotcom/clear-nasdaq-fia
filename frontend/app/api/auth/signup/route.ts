...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ok || body?.ok !== true || typeof body?.token !== "string") {
      return NextResponse.json({ ok: false, detail: body?.detail || "SIGNUP_FAILED" }, { status: upstream.status || 400 });
    }
    const result = NextResponse.json({ ok: true, user: body.user }, { status: 201, headers: { "Cache-Control": "no-store" } });
