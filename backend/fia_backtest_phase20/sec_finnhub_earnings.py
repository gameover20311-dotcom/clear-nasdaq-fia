...
POLYGON_CACHE = Path("fia_backtest_phase20/data/polygon_news_20250901_20260831.json")
OUT_PATH = Path("fia_backtest_phase20/data/earnings_events_sec_verified_20250901_20260831.json")

COMPANIES = {
    "NVDA": 1045810, "MSFT": 789019, "AAPL": 320193, "AMZN": 1018724,
    "META": 1326801, "AVGO": 1730168, "GOOGL": 1652044, "GOOG": 1652044,
    "TSLA": 1318605, "NFLX": 1065280, "AMD": 2488, "MU": 723125,
    "INTC": 50863, "QCOM": 804328, "SMCI": 1375365,
}

ALIASES = {
    "NVDA": ("nvidia", "nvda"), "MSFT": ("microsoft", "msft"),
    "AAPL": ("apple", "aapl"), "AMZN": ("amazon", "amzn"),
    "META": ("meta platforms", "meta", "facebook"), "AVGO": ("broadcom", "avgo"),
    "GOOGL": ("alphabet", "google", "googl"), "GOOG": ("alphabet", "google", "goog"),
    "TSLA": ("tesla", "tsla"), "NFLX": ("netflix", "nflx"),
    "AMD": ("advanced micro devices", "amd"), "MU": ("micron", "mu"),
    "INTC": ("intel", "intc"), "QCOM": ("qualcomm", "qcom"),
    "SMCI": ("super micro computer", "supermicro", "smci"),
}

EARNINGS_WORDS = (
    "earnings", "quarterly results", "financial results", "eps",
    "earnings per share", "revenue", "beats estimates", "misses estimates",
)

USER_AGENT = os.getenv("SEC_USER_AGENT", "CLEAR-NASDAQ-FIA historical-backtest").strip()
SEC_HEADERS = {
...
def actual_number_seen(value, article):
    text = (str(article.get("title") or "") + " " + str(article.get("description") or "")).lower().replace(",", "")
    for variant in numeric_variants(value):
        if re.search(r"(?<!\d)" + re.escape(variant) + r"(?!\d)", text):
            return True
    return False
...
def load_polygon():
    if not POLYGON_CACHE.exists():
        raise FileNotFoundError(f"Missing one-year Polygon cache: {POLYGON_CACHE}")
    ...
    prepared.sort(key=lambda a: a["_dt"])
    return prepared
    ...
    async with httpx.AsyncClient(follow_redirects=True, http2=False) as client:
        for cik, symbols in cik_to_symbols.items():
            ...
            for attempt in range(1, 5):
                response = await client.get(url, headers=SEC_HEADERS, timeout=30)
            ...
            if response is None or response.status_code != 200:
                ...
                print("SEC SKIP", "/".join(symbols), "| status =", failures[-1]["status"])
            ...
            for i, form in enumerate(forms):
                if form not in ("8-K", "8-K/A"):
                    continue
                filing_date = safe_get(recent.get("filingDate", []), i)
                if not filing_date or not (START.date().isoformat() <= filing_date <= END.date().isoformat()):
                    continue
                items = str(safe_get(recent.get("items", []), i, "") or "")
                if "2.02" not in items:
                    continue
                accepted = parse_dt(safe_get(recent.get("acceptanceDateTime", []), i))
                if accepted is None:
                    continue
                ...
                events.append({
                    ...
                    "report_date": safe_get(recent.get("reportDate", []), i),
                    "accession": safe_get(recent.get("accessionNumber", []), i),
                    "primary_document": safe_get(recent.get("primaryDocument", []), i),
                    "items": items,
                })
            await asyncio.sleep(0.35)

    events.sort(key=lambda e: e["accepted_at"])
    return events, failures


async def fetch_surprises(hub):
    key = str(hub.keys.get("FINNHUB_API_KEY") or "").strip()
    if not key:
    ...
    responses = await asyncio.gather(*(one(symbol) for symbol in COMPANIES), return_exceptions=True)
    output = {}
    plausible_start = START - timedelta(days=100)
    ...
    for result in responses:
        ...
        kept = []
        for row in rows:
            period = parse_dt(str(row.get("period") or "") + "T00:00:00+00:00")
            if period is not None and plausible_start <= period <= END:
                kept.append(row)
        output[symbol] = kept
    return output
...
def nearby_news_score(symbol, accepted_at, actual, polygon):
    ...
    return {
        "score": len(earnings_relevant) * 3 + len(relevant) + (8 if actual_confirmed else 0),
        ...
        "sample_titles": [str(a.get("title") or "") for a in earnings_relevant[:5] if a.get("title")],
    }


def match_symbol(symbol, sec_events, surprises, polygon):
    symbol_sec = [event for event in sec_events if symbol in event["symbols"]]
    rows = sorted(surprises, key=lambda row: str(row.get("period") or ""))
    used_accessions = set()
    matches, unmatched = [], []
    ...
    for surprise in rows:
        ...
        for event in symbol_sec:
            if event["accession"] in used_accessions:
                continue
            lag = (event["accepted_at"].date() - period.date()).days
            if lag < 10 or lag > 100:
                continue

            news = nearby_news_score(symbol, event["accepted_at"], surprise.get("actual"), polygon)
            timing_penalty = abs(lag - 32) / 8.0
            rank = news["score"] - timing_penalty
            candidates.append((rank, news["score"], -abs(lag - 32), event, lag, news))

        if not candidates:
            unmatched.append((surprise, "no_plausible_sec_item_2_02"))
            continue

        candidates.sort(
            key=lambda row: (row[0], row[1], row[2], row[3]["accepted_at"]),
            reverse=True,
        )
        ...
        try:
            ...
            direction = "BULLISH" if actual > estimate else "BEARISH" if actual < estimate else "NEUTRAL"
        except Exception:
            direction = "NEUTRAL"
        ...
        matches.append({
            ...
            "verification": (
                "SEC Item 2.02 acceptance timestamp gates Finnhub actual/estimate; "
                "actual is never active before reveal_at."
            ),
        })
...
def dedupe_alphabet(events):
    ...
    for event in sorted(events, key=lambda e: e["reveal_at"]):
        if event["symbol"] in ("GOOG", "GOOGL"):
            key = (event["period"], event["sec_accession"])
            if key in seen:
                continue
            seen.add(key)
            event = dict(event)
...
async def main():
    print("=== PHASE 20 ONE-YEAR SEC + FINNHUB EARNINGS ===")
    print("window =", START.isoformat(), "->", END.isoformat())
    print()

    polygon = load_polygon()
    hub = ProviderHub()
    sec_events, sec_failures = await fetch_sec_events()
    surprises = await fetch_surprises(hub)

    print("SEC Item2.02 events =", len(sec_events))
    print("Polygon cached articles =", len(polygon))
    print("SEC request failures =", len(sec_failures))
    print()

    all_matches, all_unmatched = [], []
    for symbol in COMPANIES:
        matches, unmatched = match_symbol(symbol, sec_events, surprises.get(symbol, []), polygon)
    ...
    all_matches = [
        event for event in all_matches
    ...
    OUT_PATH.write_text(
        json.dumps({
            ...
            "method": (
                "SEC 8-K Item 2.02 acceptance timestamp + Finnhub quarterly EPS surprise. "
                "Polygon historical news only resolves ambiguous SEC event matches. "
                "No future EPS is active before reveal_at."
            ),
            "events": all_matches,
    ...
    print("matched_verified_events =", len(all_matches))
    print("unmatched_surprise_rows =", len(all_unmatched))
    print("cache =", OUT_PATH)
    print()
    print("=== VERIFIED EVENTS BY SYMBOL ===")
    for symbol in [
        "NVDA", "MSFT", "AAPL", "AMZN", "META", "AVGO", "ALPHABET",
        "TSLA", "NFLX", "AMD", "MU", "INTC", "QCOM", "SMCI",
    ]:
        print(symbol, "=", by_symbol.get(symbol, 0))

    print()
    print("=== VERIFIED EVENT DATES ===")
    for event in all_matches:
        print(
            event["filing_date"], "|", event["symbol"],
            "| EPS", event["actual"], "vs", event["estimate"],
            "|", event["surprise_direction"],
            "| reveal =", event["reveal_at"],
            "| lag =", event["quarter_end_to_sec_days"],
            "| news =", event["nearby_earnings_articles"],
        )

    print()
    print("=== PHASE 20 ONE-YEAR EARNINGS CACHE COMPLETE ===")
