...
from fia_backtest_phase19.earnings_pti import earnings_point_in_time
...
async def news_asof(target, hub):
    ...
    articles=list(load_polygon_news_cache())
    polygon_status = "cached" if articles else "missing_cache"
    polygon_pages = 0
...
async def earnings_asof(target, hub):
    return earnings_point_in_time(target)
...
async def main():
    ...
    for k in ["nq_futures_price","price","nq_structure","spx_confirmation","dxy","us10y","mega_cap","semis","breadth","liquidity_direction","liquidity_signal","liquidity_conflict","news","historical_news_articles","historical_news_polygon_articles","historical_news_finnhub_articles","historical_news_kept","historical_news_directional","earnings","historical_earnings_evidence","earnings_released_events","earnings_released_symbols","earnings_upcoming_events","earnings_upcoming_symbols","earnings_upcoming_within_4h","earnings_upcoming_within_8h","earnings_upcoming_mega_weight","earnings_upcoming_semi_count","earnings_hours_to_next","earnings_catalyst_risk","earnings_future_eps_used","macro"]: print(k,"=",d.get(k))
