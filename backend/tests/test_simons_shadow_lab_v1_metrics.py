...
def test_calibration_report_is_descriptive_and_not_proven():
    report = calibration_report(_rows(), 4)
    assert report["n"] == 6
    assert report["brier_score"] is not None
    assert report["log_loss"] is not None
    assert report["ece"] is not None
    assert report["scientific_status"] == "INSUFFICIENT_SAMPLE_NOT_PROVEN"


def test_regime_and_transition_reports_keep_small_groups_exploratory():
    regimes = regime_report(_rows(), 4, min_group_n=3)
    assert {g["regime"] for g in regimes["groups"]} == {"CONFLICTED", "TREND"}
    assert all(g["status"] == "EXPLORATORY_ONLY_NOT_PROVEN" for g in regimes["groups"])

    transitions = state_transition_report(_rows(), 4, min_group_n=1)
    labels = {t["transition"] for t in transitions["transitions"]}
    assert "TREND->TREND" in labels
    assert "TREND->CONFLICTED" in labels
    assert transitions["automatic_candidate_generation"] is False


def test_parameter_robustness_never_becomes_validation():
    candidate = {
        "spec": {
            "horizon_hours": 4,
    ...
    assert report["scientific_status"] == "DISCOVERY_ROBUSTNESS_ONLY_NOT_VALIDATION"
    assert len(report["variants"]) > 1
    assert report["variants"][0]["label"] == "BASE"


def test_forward_metrics_separate_gross_from_assumed_friction():
    events = [
        {
    ...
    assert report["resolved_fires"] == 2
    assert report["gross_average_points"] == 9.0
    assert report["net_average_points_after_assumed_friction"] == 7.0
    assert report["predictive_edge_proven"] is False
    assert report["automatic_production_promotion"] is False


def test_full_diagnostic_contains_no_auto_strategy_selection():
    report = full_research_diagnostic(_rows(), 8)
    assert report["automatic_strategy_selection"] is False
    assert report["scientific_status"] == "DISCOVERY_ONLY_NOT_PROVEN"
    assert "calibration" in report and "regimes" in report and "state_transitions" in report
