...
def _protected_historical_target(output_path) -> bool:
    ...
    tooling may still write to explicit disposable paths.  The sealed historical
    CSV has no runtime/environment bypass: any deliberate offline migration must
    use a separate migration tool/path and create a new explicit scientific
    identity rather than reusing the live recorder adapter.
    """
    try:
...
def record_fia_forecast(
    ...
    Scientific-integrity boundary: the production service must never mutate the
    sealed phase14 historical_predictions.csv.  There is deliberately no env
    switch that can turn a normal request into a protected-artifact writer.
    """
