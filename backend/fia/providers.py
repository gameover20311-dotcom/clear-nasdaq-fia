...
class ProviderHub:
    ...
    async def finnhub_candles(
        ...
        # V6.6.2 FIX: the previous normalizer mapped only t/h/l and silently dropped
        # c (close), o (open) and v (volume). Every downstream structure/momentum
        # computation reads "c", so the fallback reported "N candles" while
        # delivering zero closes — which is why real candle evidence never became
        # available and the engine fell back to a QQQ quote-scalar proxy.
        timestamps = []
        highs = []
        lows = []
        closes = []
