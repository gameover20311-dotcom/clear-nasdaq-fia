...
Mirrors every appended event, its original evidence and its original head anchor
into Postgres in one transaction. Recovery restores only absent files using the
exact saved bytes. Missing historical proofs are never synthesized.
"""
...
def durability_status(root: Path | str) -> Dict[str, Any]:
    ...
    try:
        ...
        with _connect() as conn:
            ...
            with conn.cursor() as cur:
                cur.execute("SELECT COUNT(*) AS n, COUNT(*) FILTER (WHERE is_test) AS t "
                            "FROM forward_oos_events WHERE campaign_id=%s", (cid,))
                row = cur.fetchone() or {}
        saved = {item["row"]["file_name"]: bytes(item["row"]["canonical_json"]) for item in archive}
...
def mirror_event(root: Path | str, event: Dict[str, Any], canonical: bytes,
    ...
    try:
        ...
        with _connect() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("INSERT INTO forward_oos_bundles "
                            ...
                            "VALUES (%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (campaign_id, seq) DO NOTHING",
        ...
        print("Forward-OOS durable mirror FAILED -> %s (event exists on ephemeral storage only)" % type(exc).__name__)
        return {"mirrored": False, "reason": "MIRROR_FAILED", "error_type": type(exc).__name__}
...
def restore_missing(root: Path | str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"ok": False, "restored": 0, "reason": "RESTORE_FAILED", "error_type": type(exc).__name__}
...
def _retry_local_tip(root: Path, rows, bundles, archive) -> bool:
    ...
    count = len(local)
    if (not count or len(archive) != len(rows) or count not in (len(rows), len(rows) + 1) or
...
def write_test_fixture(root: Path | str, marker: str) -> Dict[str, Any]:
    ...
    try:
        ...
        payload = json.dumps({"marker": marker, "purpose": "durability probe", "scoreable": False}, sort_keys=True).encode()
        with _connect() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT COALESCE(MIN(seq), 0) AS m FROM forward_oos_events WHERE campaign_id=%s AND is_test", (cid,))
                nxt = int((cur.fetchone() or {}).get("m") or 0) - 1
                cur.execute(
                    "INSERT INTO forward_oos_events (campaign_id, seq, event_type, forecast_id, created_at_utc, "
                    "prev_event_hash, event_hash, file_name, canonical_json, is_test) "
        ...
        with _connect() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT seq, event_type, mirrored_at FROM forward_oos_events "
                            "WHERE campaign_id=%s AND is_test AND forecast_id=%s", (cid, marker))
                row = cur.fetchone()
        return {"found": bool(row), "marker": marker, "seq": (row or {}).get("seq"),
...
def delete_test_fixture(root: Path | str, marker: str) -> Dict[str, Any]:
    ...
    try:
        ...
        with _connect() as conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM forward_oos_events WHERE campaign_id=%s AND is_test = TRUE AND forecast_id=%s",
                            (cid, marker))
                n = cur.rowcount
            conn.commit()
        return {"deleted": int(n), "marker": marker, "production_rows_touched": 0}
    except Exception as exc:
