...
def _draft_declarations():
    return DeclarationBundle(
        ...
        gamma_theta_id="UNSET_GAMMA_REQUIRES_FREEZE", phi_id="UNSET_PHI_REQUIRES_FREEZE",
        delta_stop_id="UNSET_STOP_REQUIRES_FREEZE", bellman_policy_id="UNSET_BELLMAN_REQUIRES_FREEZE",
    )
...
def _frozen_fixture():
    return DeclarationBundle(
        ...
        compute_budget=2, acquisition_budget=2, selection_budget=2,
        gamma_theta_id="TEST_ONLY_GAMMA", phi_id="TEST_ONLY_PHI", delta_stop_id="TEST_ONLY_STOP", bellman_policy_id="TEST_ONLY_BELLMAN",
        ...
        k_max=2, c_a=0.3, tie_break=("stop","acquire","compute"),
...
class MFRETruthStatusTests(unittest.TestCase):
    def test_missing_declarations_never_looks_ready(self):
        ...
        self.assertEqual(s["declaration_status"],"MISSING"); self.assertEqual(s["run_status"],"INERT_DECLARATIONS_MISSING")
        self.assertFalse(s["shadow_runtime_ready"]); self.assertFalse(s["dashboard_decision_ready"])
    ...
    def test_unfrozen_declarations_never_get_protocol_identity(self):
        ...
        self.assertEqual(s["declaration_status"],"UNFROZEN"); self.assertEqual(s["run_status"],"INERT_DECLARATIONS_UNFROZEN"); self.assertIsNone(s["protocol_identity"])

    def test_dpcse_not_armed_is_visible_blocker(self):
        s=build_truth_status(_frame(),_frozen_fixture())
        self.assertIn("DPCSE_CANDIDATE_NOT_FROZEN",s["research_blockers"]); self.assertIn("DPCSE_NOT_ARMED",s["research_blockers"]); self.assertFalse(s["shadow_contract_ready"])
    ...
    def test_shadow_or_umse_failure_is_visible(self):
        ...
        self.assertIn("SHADOW_LAB_UNAVAILABLE",s["research_blockers"]); self.assertIn("UMSE_INTEGRITY_FAIL",s["research_blockers"]); self.assertFalse(s["shadow_runtime_ready"])
    ...
    def test_contract_ready_still_reports_execution_engine_unbound(self):
        ...
        self.assertEqual(s["run_status"],"READY_SHADOW_CONTRACT_ONLY"); self.assertTrue(s["shadow_contract_ready"]); self.assertFalse(s["execution_engine_bound"]); self.assertFalse(s["shadow_runtime_ready"])
        self.assertIn("MFRE_EXECUTION_ENGINE_NOT_YET_BOUND",s["research_blockers"]); self.assertFalse(s["dashboard_decision_ready"])

    def test_status_never_claims_edge_or_production_authority(self):
        s=build_truth_status(_frame(dpcse_frozen=True,dpcse_status="ARMED_N0"),_frozen_fixture())
        self.assertEqual(s["theory_freeze"],"FINAL_FREEZE_PASS"); self.assertEqual(s["implementation_scope"],"RESEARCH_SHADOW_ONLY"); self.assertFalse(s["production_authorized"])
        self.assertEqual(s["decision_authority"],"NONE"); self.assertEqual(s["predictive_edge"],"NOT_PROVEN"); self.assertEqual(s["real_market_edge"],"NOT_TESTED"); self.assertEqual(s["mfre_incremental_value"],"NOT_TESTED")
        self.assertIsNone(s["directional_override"]); self.assertIn("MFRE_PRODUCTION_AUTHORITY_NOT_GRANTED",s["decision_blockers"]); self.assertIn("PREDICTIVE_EDGE_NOT_PROVEN",s["decision_blockers"])
