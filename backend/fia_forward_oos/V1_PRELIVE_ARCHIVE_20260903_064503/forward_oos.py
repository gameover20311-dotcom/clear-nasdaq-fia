...
SCHEMA_VERSION = "SOL56_FORWARD_OOS_V1"
...
def model_fingerprint(backend_root: Path = ROOT) -> Dict[str, Any]:
    files: Dict[str, str] = {}
    for rel in CORE_FINGERPRINT_FILES:
        path = backend_root / rel
        files[rel] = sha256_file(path) if path.exists() else "MISSING"
    digest = sha256_bytes(canonical_bytes(files))
    return {"algorithm": "sha256", "digest": digest, "files": files}
...
def build_campaign_seal(now: Optional[datetime] = None) -> Dict[str, Any]:
    ...
    unsigned = {
        ...
        "campaign_id": "SOL56-NEW-FORWARD-OOS-V1",
...
def _forecast_values(forecast: Any) -> Dict[str, Any]:
    ...
    return {
        ...
        # BASE_FIA currently emits one pre-move probability distribution that is
        # evaluated at both 4H and 8H. Preserve both horizon fields explicitly
        # instead of pretending a separately-trained 4H probability exists.
...
def _completed_close_from_polygon_payload(payload: Any, as_of: datetime, max_age_minutes: float) -> Optional[Dict[str, Any]]:
    if not isinstance(payload, dict):
        return None
    ts = payload.get("t") or []
    closes = payload.get("c") or []
    best = None
    for raw_ts, raw_close in zip(ts, closes):
        try:
            start = datetime.fromtimestamp(float(raw_ts), tz=timezone.utc)
            end = start + timedelta(minutes=5)
            close = float(raw_close)
        except Exception:
            continue
        if not math.isfinite(close) or close <= 0 or end > as_of:
            continue
        if best is None or end > best[0]:
            best = (end, close)
    if best is None:
        return None
    age_min = (as_of - best[0]).total_seconds() / 60.0
    if age_min < -1e-9 or age_min > max_age_minutes:
        return None
    return {"bar_end_utc": _iso(best[0]), "close": round(best[1], 6), "age_minutes": round(age_min, 3)}
...
async def explicit_contract_completed_close(
    ...
    """Get the newest completed explicit-contract 5m close at/before ``as_of``."""
    ...
    try:
        ...
        return None
    return _completed_close_from_polygon_payload(payload, as_of, max_age_minutes)
...
async def lock_live_forecast(
    ...
    base = _forecast_values(forecast)
    ...
    require_explicit = str(os.getenv("FIA_FORWARD_OOS_REQUIRE_EXPLICIT_CONTRACT", "1")).lower() not in {"0", "false", "no"}
    if require_explicit and not contract_meta["explicit"]:
