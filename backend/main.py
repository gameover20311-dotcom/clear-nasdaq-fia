...
def compare_fia_with_gemini(fia, chart_analysis):
    ...
    # SOL56_COMPARISON_TRUTH_V1
    # A direction-only chart analysis does not contain a probability estimate.
    # Do not invent a 50% probability and then award probability-proximity points
    # for fabricated evidence. Keep it unavailable; the comparison can still use
    # direction and explicitly supplied confidence.
    if gemini_bullish is None and gemini_bearish is None:
        gemini_bullish = None
        gemini_bearish = None
...
@app.post("/api/chart/analyst-report")
async def chart_analyst_report(
    file: UploadFile = File(...),
    user_direction: str = Form(""),
    user_timeframe: str = Form(""),
    user_reasoning: str = Form(""),
):
    """Full Chart Analyst: independent vision read + real FIA context.

    The vision leg runs FIRST and is never shown the FIA direction, the FIA
    probability or the user's opinion. Only afterwards is its output compared
    against verified backend evidence. Chart-derived claims and backend facts
    are returned in separate, origin-tagged blocks so the UI cannot merge them.

    This endpoint is read-only with respect to the pre-move ledger: build_watch
    is called with persist=False so uploading a chart never writes a forecast
    observation.
    """
    allowed = {"image/png", "image/jpeg", "image/webp"}
    if file.content_type not in allowed:
    ...
    # --- 1. INDEPENDENT vision. No FIA/user context is passed in. ---------
    try:
        vision = await analyze_advanced_chart_independent(
            image_bytes=image_bytes,
            content_type=file.content_type,
            filename=file.filename or "chart",
        )
    except Exception as exc:
        raise HTTPException(status_code=502, detail="Vision analysis unavailable: %s" % exc)

    user_analysis = {
        "direction": (user_direction or "").strip().upper(),
        "timeframe": (user_timeframe or "").strip(),
        "reasoning": (user_reasoning or "").strip(),
    }

    # --- 2. Real backend evidence. Failures stay visible, never neutral. --
    premove = None
    cognitive = None
    three_way = None
    forecast_payload = {}
    ...
    try:
        ...
        forecast_payload = forecast.model_dump() if hasattr(forecast, "model_dump") else dict(
            getattr(forecast, "__dict__", {}) or {}
        )
        ...
        try:
            ...
            premove = build_watch(
                ...
                persist=False,          # read-only: never append a ledger row
            )
        except Exception as exc:
            backend_errors.append("premove_watch: %s" % exc)

        try:
            cognitive = build_cognitive_report(forecast_input, forecast, horizon="8h", persist=False)
        except Exception as exc:
            backend_errors.append("cognitive: %s" % exc)

        try:
            accuracy_assessment = build_accuracy_assessment(forecast, data)
            phase25 = build_confluence_assessment(forecast, accuracy_assessment, vision, data)
            three_way = build_three_way_confluence(forecast, user_analysis, vision, phase25)
            three_way["phase25"] = phase25
        except Exception as exc:
            backend_errors.append("three_way: %s" % exc)

    except Exception as exc:
        backend_errors.append("snapshot: %s" % exc)

    report = build_chart_analyst_report(
        vision=vision,
    ...
    if backend_errors:
        # Surfaced, not swallowed. Absent evidence must remain visibly absent.
        report["backend_evidence_errors"] = backend_errors
    return report
...
# Separate append-only, hash-chained forward ledger. Historical/legacy holdouts
# are explicitly ineligible for promotion. The background worker never backfills
# a missed checkpoint.
from fia.forward_oos_api import install_forward_oos_routes
install_forward_oos_routes(app, hub, build_forecast)


# CLEAR_NASDAQ_WHOLE_SYSTEM_BACKTEST_FINAL_V2
try:
    from fia_whole_system_backtest.api import install_whole_system_backtest_routes
    install_whole_system_backtest_routes(app, Path(__file__).resolve().parent)
except Exception as _whole_bt_error:
    print('WHOLE_SYSTEM_BACKTEST_ROUTE_INSTALL_ERROR', repr(_whole_bt_error))


# CLEAR_NASDAQ_FINAL_FUSION_4H_8H_OVERNIGHT_V1
try:
    from fia_final_cockpit.api import install_final_cockpit_routes
    install_final_cockpit_routes(app, hub, build_forecast, Path(__file__).resolve().parent)
except Exception as _final_cockpit_error:
    print('FINAL_COCKPIT_ROUTE_INSTALL_ERROR', repr(_final_cockpit_error))

# === CLEAR NASDAQ OBSIDIAN FULL ACCESS AUTH V3 ===
from fia.auth_api import install_auth_routes as _install_auth_routes
