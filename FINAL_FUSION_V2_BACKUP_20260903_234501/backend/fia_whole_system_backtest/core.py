...
def extract_outcome_direction(row: Mapping[str,Any], hours: int=8) -> Tuple[Optional[str],str]:
    ...
    for p,v in flat:
        ...
        if d not in DIRECTIONAL: continue
        score=0
        if _horizon_match(pl,hours): score+=30
        if any(t in pl for t in OUTCOME_TOKENS): score+=25
        if 'direction' in pl: score+=10
        if 'prediction' in pl or 'forecast' in pl: score-=20
        if score>20: candidates.append((score,d,p))
    if candidates:
    ...
    if entry is not None and future is not None and future!=entry:
        return ('BULLISH' if future>entry else 'BEARISH'), f'{entry_path} -> {future_path}'
...
def nearest_trace_outcome(ts: str, trace: Sequence[Mapping[str,Any]], hours: int=8) -> Tuple[Optional[str],str]:
    ...
    for row in trace:
        ...
        if d in DIRECTIONAL and (best is None or delta<best[0]): best=(delta,d,'trace:'+src)
...
def score_records(records: Sequence[Mapping[str,Any]]) -> Dict[str,Any]:
    wins=losses=no_edge=fail_closed=unresolved=0
    briers=[]; directional=[]; all_eligible=0
    calibration=defaultdict(lambda:{'n':0,'p_sum':0.0,'y_sum':0.0})
    by_week=defaultdict(lambda:{'wins':0,'losses':0,'no_edge':0,'fail_closed':0,'eligible':0})
    by_month=defaultdict(lambda:{'wins':0,'losses':0,'no_edge':0,'fail_closed':0,'eligible':0})
    for r in records:
        out=normalize_direction(r.get('outcome_direction'))
        if out not in DIRECTIONAL:
        ...
        if status!='OK':
            ...
            if d==out:
                ...
                losses+=1; by_week[week]['losses']+=1; by_month[month]['losses']+=1
    ...
    return {
        ...
        'truth_note':'Win rate = wins / (wins + losses) only. NO_EDGE, FAIL_CLOSED and unresolved are shown separately; directional coverage is always displayed to prevent abstention inflation.',
    }
...
def make_dual_report(backend:Path,brain_root:Path,history_path:Path,brain_records_path:Path,dual_records_path:Path,master_result:Dict[str,Any],state:str,total:int,completed:int,brain_fp:Optional[Dict[str,Any]]=None,reused_brain_records:int=0,new_brain_runs:int=0) -> Dict[str,Any]:
    ...
    report={
        ...
        'truth_policy':{
            ...
            'checkpoint_hash_chain_required':True,
            'brain_fingerprint_bound_to_case_identity':True,
            'win_rate_formula':'wins/(wins+losses) for resolved directional calls at each horizon; coverage and abstentions are always shown.',
        },
        'known_historical_limitations':[
