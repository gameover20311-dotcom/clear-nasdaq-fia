...
def main()->int:
    ...
    fp=brain_fingerprint(brain); tag=fp['digest'][:12]
    # IMPORTANT: keep the original 8H checkpoint filename so the four already-completed cases are automatically reused.
    ...
    for row in rows:
        ...
        if not ts: continue
        cid=case_id(ts,row,fp['digest'])
        outcomes={}
        for h in (4,8):
            out,src=extract_outcome_direction(row,h)
            if out not in {'BULLISH','BEARISH'}: out,src=nearest_trace_outcome(ts,trace,h)
            outcomes[f'{h}h']={'direction':out if out in {'BULLISH','BEARISH'} else None,'source':src}
    ...
    print('Historical rows:',history_path);print('Eligible timestamp cases:',total);print('Already dual-scored/resumable:',len(scored_done));print('Existing V6.6 brain-cache records:',len(brain_cache));print('Brain fingerprint:',fp['digest']);print('Truth: one locked V6.6 forecast; genuine 4H + 8H market resolutions; no future leakage; missing is not faked');print()
    ...
    for idx,(cid,ts,row,outcomes) in enumerate(cases,1):
        ...
        elif not any(outcomes[k]['direction'] in {'BULLISH','BEARISH'} for k in ('4h','8h')):
    ...
    final=make_dual_report(backend,brain,history_path,brain_records,dual_records,master,'COMPLETE',total,len(scored_done),fp,reused,new_runs); final['history_mode']=history_mode; final['brain_version']=getattr(__import__('fia_brain'),'__version__','unknown'); atomic_write_json(report_path,final)
    atomic_write_json(progress_path,{'state':final.get('state'),'final':bool(final.get('final')),'completed':len(scored_done),'total':total,'reused_brain_records':reused,'new_brain_runs':new_runs,'updated_at_utc':utc_now(),'report_sha256':final['report_sha256'],'brain_fingerprint':fp['digest']})
