...
class ProviderHub:
    ...
    async def get_nq_session_liquidity_yahoo(self):
        """
        NQ=F Yahoo liquidity engine.
        Gets real futures candles and calculates Asia/London/New York highs/lows.
        """
        ...
        try:
            ...
            for name, values in sessions.items():
                ...
                if not session.empty:
                    output[f"{name}_high"] = float(
                        session["High"].max()
                    )
                    output[f"{name}_low"] = float(
                        session["Low"].min()
                    )
            ...
            print("NQ YAHOO LIQUIDITY:", output)
    ...
    async def snapshot(self) -> Dict[str, Any]:
        ...
        if self.keys["FRED_API_KEY"]:
            # 0.0 is a real neutral observation only when the provider exists.
            data["macro"] = 0.0
            data["macro_status"] = "FRED live / calendar neutral"
        else:
            data["macro"] = None
            data["macro_status"] = "missing"

        # =====================================================
        # Liquidity — Phase 22 instrument separation
        # =====================================================

        try:
            liquidity_data = await self.liquidity("QQQ")
            ...
            for key, value in (liquidity_data or {}).items():
                if value is None:
                    continue
                try:
                ...
                # Preserve legacy top-level keys for old dashboard/backtests.
                data[key] = numeric

                if key in period_keys:
                    qqq_levels[key] = numeric
                elif key in session_keys:
                    nq_levels[key] = numeric

            data["qqq_liquidity"] = {
                "instrument": "QQQ",
                "current_price": data.get("price"),
                "levels": qqq_levels,
                "source": "QQQ daily candles",
            }
            data["nq_liquidity"] = {
                "instrument": "NQ",
                "current_price": data.get("nq_futures_price"),
                "levels": nq_levels,
                "source": "NQ futures 5m session candles",
            }
            data["liquidity_evidence"] = {
                "qqq": "available" if qqq_levels else "missing",
                "nq": "available" if nq_levels else "missing",
            }
            data["liquidity_evidence_available"] = bool(qqq_levels and nq_levels)

        except Exception as e:
            data["qqq_liquidity"] = {
                "instrument": "QQQ", "current_price": data.get("price"),
                "levels": {}, "source": "missing",
            }
            data["nq_liquidity"] = {
                "instrument": "NQ", "current_price": data.get("nq_futures_price"),
                "levels": {}, "source": "missing",
            }
            data["liquidity_evidence"] = {"qqq": "missing", "nq": "missing"}
            data["liquidity_evidence_available"] = False
            print(f"Liquidity intelligence error -> {e}")
